...
export default function PublicTransportInterface() {
  ...
  // Liste des partenaires (isolée pour le doublement nécessaire au scroll infini)
  const partners = [
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 antialiased selection:bg-orange-500 selection:text-white scroll-smooth">
      ...
      {/* SECTION LOGOS PARTENAIRES (MODIFIÉE POUR LE SCROLL INFINI VERS LA DROITE) */}
      <section className="bg-white border-b border-slate-100 py-10 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 md:px-12 text-center">
          ...
          <div className="relative w-full overflow-hidden flex [mask-image:linear-gradient(to_right,transparent,white_15%,white_85%,transparent)]">
            ...
            {/* Conteneur animé avec Framer Motion se déplaçant de -50% à 0% (vers la droite) */}
            <motion.div 
            ...
            >
              ...
              {partners.map((partner, idx) => (
                <span key={`p1-${idx}`} className={partner.className}>
                  {partner.name}
                </span>
              ))}
              {/* Deuxième groupe identique pour assurer la boucle infinie visuelle */}
              {partners.map((partner, idx) => (
                <span key={`p2-${idx}`} className={partner.className}>
                  {partner.name}
                </span>
              ))}
            </motion.div>
