...
export default function AuthInterface({ onBack }) {
  ...
  {/* 1. Votre bouton stylisé (celui qui est visible par l'utilisateur) */}
  <div className="w-full flex items-center justify-center gap-3 border border-slate-300 text-slate-600 font-medium py-3 rounded-full hover:bg-slate-50 transition-all text-sm cursor-pointer shadow-sm">
  ...
  {/* 2. Le composant GoogleLogin invisible posé par-dessus */}
  <div className="absolute inset-0 opacity-0 cursor-pointer">
    <GoogleLogin
      onSuccess={async (credentialResponse) => {
        try {
          setIsLoading(true);
          // On envoie le token reçu à votre backend
